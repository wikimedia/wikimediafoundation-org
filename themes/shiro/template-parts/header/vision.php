...
foreach ( $visions as $key => $vision ) {
	...
	?>
	<<?php echo sanitize_key( $element_tag ); // phpcs:ignore WordPress.Security.EscapeOutput ?> aria-hidden="<?php echo esc_attr( (bool) $vision_output['aria_hidden'] ? 'true' : 'false' ); ?>" lang="<?php echo esc_attr( substr( $vision_output['langcode'], 0, 2 ) ); ?>" class="<?php echo implode( ' ', array_map( 'esc_attr', array_filter( $vision_output['classes'] ) ) ); ?>">
		<?php echo esc_html( $vision_output['text'] ); ?>
	</<?php echo sanitize_key( $element_tag ); // phpcs:ignore WordPress.Security.EscapeOutput ?>>
	<?php
}
