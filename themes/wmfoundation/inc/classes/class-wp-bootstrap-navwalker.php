...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			// with pointer at end of array check if we got an ID match and build a string to use as aria-labelledby.
			$labledby = end( $matches[2] ) ? 'aria-labelledby="' . end( $matches[2] ) . '"' : '';

			$class = $depth ? 'nav flex-column' : 'dropdown-menu';

			if ( ! empty( $args->row_depth ) ) {
				$args->row_depth++;
			}

			if ( empty( $args->has_grandchildren ) ) {
				$output .= "\n$indent<ul role=\"menu\" class=\" $class\" " . esc_attr( $labledby ) . ">\n";
			} else {
				$args->row_depth = 1;
				$output         .= "\n$indent<ul role=\"menu\" class=\" $class dropdown-2-column\" " . esc_attr( $labledby ) . "><div class=\"row m-0\">\n";
			}
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			// initiate empty icon var then if we have a dropdown ...
			$dropdown_icon_html = '';
			$bottom_border      = '';

			if ( 'dropdown-toggle nav-link' === $atts['class'] ) {
				// append an <i> with the icon classes to what is output before links.
				$dropdown_icon_html = bisk_get_icon( 'util-chevron' );
				$bottom_border      = '<div class="bottom-border d-none"></div>';
			}
			$item_output .= $args->link_before . $icon_html . apply_filters( 'the_title', $item->title, $item->ID ) . $dropdown_icon_html . $args->link_after;
			$item_output .= '</a>' . $bottom_border;
		...
		public function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
			...
			if ( is_object( $args[0] ) ) {
				...
				$args[0]->has_grandchildren = false;

				if ( ! empty( $children_elements[ $element->$id_field ] ) ) {
					$args[0]->has_grandchildren = $this->has_grandchildren( $children_elements[ $element->$id_field ], $children_elements );
				}
			}
		...
		/**
		 * Checks to see if the current element has grandchildren.
		 *
		 * @param array $current_children_elements Children for the current element.
		 * @param array $children_elements         All children elements.
		 *
		 * @return bool
		 */
		public function has_grandchildren( $current_children_elements, $children_elements ) {

			$id_field = $this->db_fields['id'];

			foreach ( $current_children_elements as $child ) {
