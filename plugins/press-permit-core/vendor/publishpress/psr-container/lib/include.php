...
if (! function_exists(__NAMESPACE__ . '\register2Dot0Dot1Dot9')) {
    ...
    add_action('plugins_loaded', __NAMESPACE__ . '\register2Dot0Dot1Dot9', -200, 0);

    function register2Dot0Dot1Dot9()
    {
        if (! interface_exists('PublishPress\Psr\Container\ContainerInterface')) {
            $versions = Versions::getInstance();
            $versions->register('2.0.1.9', __NAMESPACE__ . '\initialize2Dot0Dot1Dot9');
        }
    }

    function initialize2Dot0Dot1Dot9()
    {
        require_once __DIR__ . '/autoload.php';
        
        if (! defined('PUBLISHPRESS_PSR_CONTAINER_VERSION')) {
            define('PUBLISHPRESS_PSR_CONTAINER_VERSION', '2.0.1.9');
        }
        
        do_action('publishpress_psr_container_2Dot0Dot1Dot9_initialized');
    }
}
