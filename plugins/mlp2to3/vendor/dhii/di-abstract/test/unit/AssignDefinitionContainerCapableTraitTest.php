...
use Dhii\Di\AssignDefinitionContainerCapableTrait as TestSubject;
 ...
 * @since [*next-version*]
 */
class AssignDefinitionContainerCapableTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Di\AssignDefinitionContainerCapableTrait';
     ...
     * @since [*next-version*]
     *
     * @return MockObject|callable An object that has an `__invoke()` method.
     */
    ...
    public function testAssignDefinitionContainer()
    {
        ...
        $subject->expects($this->exactly(1))
            ...
            ->with($definition, [$container1, $arg0, $arg1])
            ->will($this->returnCallback(function ($definition, $args) {
                return call_user_func_array($definition, $args);
            }));

        $wrapped = $_subject->_assignDefinitionContainer($definition, $container2);
        $result = call_user_func_array($wrapped, [$container1, $arg0, $arg1]);
        $this->assertEquals($service, $result, 'Wrong result produced by wrapped definition');
    }
}
