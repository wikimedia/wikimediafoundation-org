...
class OverrideProductGallery
{
    ...
    {
        $key = MetaboxFields::FIELD_OVERRIDE_PRODUCT_GALLERY;

        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
        ?>
        <tr>
            <th scope="row">
                <?= esc_html_x('Gallery', 'WooCommerce Product Field', 'multilingualpress') ?>
            </th>
            <td class="form-field <?= $key ?>_field">
                <label for="<?= esc_attr($helper->fieldId($key)) ?>">
                    ...
                    />
                    <?= esc_html_x(
                        "Override the translation's gallery with the gallery of this product.",
